...
import ClickBtn from "../shared/ClickBtn";
import NotFound from "../notfoundpage/NotFound";
import loading from './../../app/friend-details/[id]/loading';
...
const DetailsCard = ({ id }) => {
  ...
  const filterData = friends.find((friend) => friend.id == id);
  if(!filterData){
    if(loading){
  ...
  return (
    <div className="container flex flex-col gap-4 mx-auto my-5 px-3 sm:px-4">
      <div className="grid gap-4 md:grid-cols-3 md:grid-rows-[1fr_1fr]">
        ...
        <div className="md:col-span-2 md:row-span-1 grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="text-center shadow bg-white rounded-md flex items-center justify-center p-4 col-span-2 sm:col-span-1">
            <div>
              <h2 className="font-bold text-lg md:text-2xl">{next_due_date}</h2>
      ...
      <div className="grid md:grid-cols-3 md:grid-rows-1 gap-4">
        ...
        >
          <div className="space-y-2">
            <h2 className="font-bold text-xl text-[#434953]">Quick Check-In</h2>
